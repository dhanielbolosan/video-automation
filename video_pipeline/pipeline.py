...
def build_video(
    *,
    ...
    notes = source_path.read_text(encoding="utf-8") if source_path.is_file() else ""
    if plan_path:
        plan = load_plan(plan_path, frame_count)
    else:
        if not notes.strip():
            raise ValueError("source notes are empty; provide --source or --plan")
        plan = make_plan(topic, notes, frame_count, length, model, subscription)
    add_frame_ids(plan)
    ...
    node("transitions", "inject", "--storyboard", "./STORYBOARD.md", "--hyperframes", ".", cwd=project)
    node("transitions", "verify", "--storyboard", "./STORYBOARD.md", "--index", "./index.html", cwd=project)

    hyperframes("lint", cwd=project, timeout=1800)
    ...
    result: dict[str, object] = {
        ...
        "model_calls": 0 if plan_path else 1,
    }
    if render:
