...
from .runtime import PACKAGE_DIR, ROOT, slugify

RESEARCH_DIR = ROOT / "research"
PROMPT = PACKAGE_DIR / "prompts" / "research.md"

# JSON schema that constrains the research agent's response.
def schema() -> dict:
    return {
        "type": "object",
...
# Keep only complete, sourced facts and assign stable fact identifiers.
def validate(
    payload: dict,
...
def load_research(path: Path, expected_topic: str | None = None) -> dict:
    if not path.is_file():
...
# Extract the structured JSON payload returned by a Claude command.
...
# Load cached research or perform and cache a fresh research run.
def get_research(
    topic: str,
    audience: str,
    model: str,
    ...
    if path.is_file() and not refresh:
        cached = load_research(path, topic)
...
# Convert sourced research facts into the planner's compact note format.
...
# Save the research JSON and a human-readable source ledger in the project.
